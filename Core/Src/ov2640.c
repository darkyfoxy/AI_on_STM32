...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, TEST_OV, TEST_HSIZE_SET(w_mul));
    ...
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, RESET_OV, 0x00);
...
static int set_auto_gain(sensor_t *sensor, int enable, float gain_db, float gain_db_ceiling)
{
    ...
    if ((enable == 0) && (!isnanf(gain_db)) && (!isinff(gain_db))) {
        ...
        int gain_temp = roundf(fast_log2(IM_MAX(gain / 2.0, 1.0)));
        ...
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM9, (reg & 0x1F) | ((int)(ceilf(fast_log2(gain_ceiling)) - 1) << 5));
    }
...
static int sensor_snapshot(DCMI_HandleTypeDef DCMIHandle, int32_t *image)
{
	...
	HAL_DCMI_Start_DMA(&DCMIHandle, DCMI_MODE_SNAPSHOT, (uint32_t) image, RGB565_VGA_BUF_SIZE/4);
	#endif

    return 0;
}

static int set(sensor_t *sensor)
{
	int ret;
	ret = sensor->set_pixformat(sensor, sensor->pixformat);
	ret |= sensor->set_framesize(sensor, sensor->framesize);
	ret |= sensor->set_contrast(sensor, sensor->contrast_level);
	ret |= sensor->set_brightness(sensor, sensor->brightness_level);
	ret |= sensor->set_contrast(sensor, sensor->saturation_level);
	ret |= sensor->set_gainceiling(sensor, GAINCEILING_128X);
	return ret;
}
